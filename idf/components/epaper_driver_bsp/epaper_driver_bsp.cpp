/*
 * Драйвер панели 1.54" 200x200 (SSD1681) — взят из примеров Waveshare
 * (`02_Example/ESP-IDF/V2/12_RTC_Sleep_Test/components/epaper_driver_bsp`), 2026-09-16.
 *
 * Наши правки (помечены «ПРАВКА INKMETRICS»), остальное — как у Waveshare:
 *   1. буфер кадра берётся из внутренней ОЗУ (MALLOC_CAP_DEFAULT), а не из PSRAM:
 *      в нашем sdkconfig PSRAM не включена, а 5000 байт кадра во внутренней ОЗУ не жалко;
 *   2. добавлен доступ к кадровому буферу (EPD_Buffer) — рисуем текст и графику прямо в него.
 * Проверенные волновые таблицы (159 значений) — то, на чём прошлый самодельный драйвер
 * спотыкался (152 значения → панель не могла завершить обновление, BUSY висел навсегда).
 */
#include <stdio.h>
#include <string.h>
#include <math.h>
...
epaper_driver_display::epaper_driver_display(int width, int height,custom_lcd_spi_t _lcd_spi_data) : 
    ...
    /* ПРАВКА INKMETRICS: было MALLOC_CAP_SPIRAM — в нашей сборке PSRAM не включена */
