...
| Сборка со счётчиками кадров и событиями USB | **собрана, на железо не заливалась** (`idf/build`): счётчики приёма/отдачи, события `прицепился/отцепился`, строка раз в 10 с, подробные логи сети |
| **Самопроверка: уход в загрузчик без кнопки BOOT** | **собрано, на железо не заливалась** (`idf/build`, образ 470 496 Б): пинг хоста, учёт кадров/DHCP/HTTP, автоматический уход в загрузчик после 5 мин молчания — см. раздел ниже |

**Что нового в этой сборке (2026-09-16, вечер).** Счётчики кадров (`tud_network_recv_cb` /
`usb_transmit`) и строка раз в 10 с (`ip`, `link`, кадры, heap) отвечают на вопрос «доходят ли
до прибора пакеты хоста»; события `ATTACHED/DETACHED` теперь дёргают
`esp_netif_action_connected/disconnected`; строки тегов сети (`esp_netif`, `dhcps`, `tusb_net`)
сохраняются в «чёрный ящик» в любом уровне, для чего поднят `CONFIG_LOG_MAXIMUM_LEVEL_DEBUG`
(без него `ESP_LOGD` вырезается компилятором). Ящик расширен до 16 КБ (стирание по сектору).
...
## Самопроверка: прибор сам уходит в режим загрузки (2026-09-16)

Задача от пользователя: «добиться втыкания в хост и веб-сервера, а если связи нет — прибор
пусть сам уходит в режим прошивки, чтобы не нажимать BOOT; в финальной версии отключим».

Как сделано (`idf/main/selfcheck.c`, описание констант — `selfcheck.h`):

* каждые 3 с прибор пингует хост (по умолчанию `192.168.7.2`, а как только из ARP-запроса
  узнаёт настоящий адрес хоста — пингует его);
* признаками жизни считаются **любой** кадр от хоста, DHCP-пакет от него (хост просит адрес),
  ответ на ping и любой HTTP-запрос к нашему серверу — так сделано потому, что ICMP на Windows
  по умолчанию может быть закрыт файрволом, а ARP/DHCP/HTTP работают всегда;
* если за `SELFCHECK_FALLBACK_MS` (5 минут) признаков не было ни одного — `diag_step` + запись
  RTC-бита `FORCE_DOWNLOAD_BOOT` + `esp_restart()`: чип уходит в загрузчик сам;
* **в релизе выключается** одной константой: `SELFCHECK_ENABLED 0` в `idf/main/selfcheck.h`.

Что видно в веб-сервере (страница `/` и `/api/state`): адрес хоста и откуда он узнан, возраст
последнего ответа на ping и счётчики ok/таймаут, число кадров от хоста и DHCP-пакетов,
запросы к серверу, сколько секунд хост молчит и сколько осталось до ухода в загрузчик.
С 17.09 там же **метрики хоста**, присланные агентом (`POST /ingest`): на странице строки
«Метрики хоста (агент)» и «Хост: пинг, TCP, SMART», в JSON — объект `ingest`; на экране
прибора это страница 5/5 `HOST SYS`. Подробности — `docs/ingest-2026-09-17.md`.
Ссылка «уйти в режим загрузки сейчас» (`GET /api/boot`) на странице есть, но **в загрузчик она
не уводит** (проверено 17.09): работает зажатие `BOOT` при подключении USB.

**Важно: «липкий» RTC-бит.** Пока бит установлен, ROM уходит в загрузчик при каждом сбросе,
поэтому после записи прошивки плата снова оказалась бы в загрузчике, пока не передёрнешь USB.
Поэтому:

* `tools/try_flash.py` после записи сам снимает бит (`esptool write-mem 0x6000812C 0x00`)
  и сбрасывает чип — прибор сразу запускается из приложения, передёргивать USB не нужно;
* в `flash-kit/idf/flash*.bat` те же два шага добавлены в конце;
* приложение при старте тоже чистит бит (`app_main`) — на случай, если он остался от прошлого
  цикла, но чип всё-таки запустил приложение.

**Ограничение:** если приложение само перезагружается в цикле (З-01), таймер 5 минут не
досчитывает — самопроверка до загрузчика не доведёт. Причина перезагрузок всё ещё не снята.
### «Чёрный ящик» против цикла перезагрузок (2026-09-16, позже вечером)

Лог каждой загрузки стирается, поэтому в цикле перезагрузок его почти не видно. Добавлено два
средства:

* **журнал загрузок** (`diag_journal_add()` в `diag.c`, адрес `0x7E4000` — второй сектор раздела,
  вне стираемой области): при каждой загрузке в свободную (0xFF) ячейку дописывается 16 байт —
  номер загрузки, причина сброса, когда поднялся ящик. Ничего не стирается, 256 записей.
  `tools/idf_diag.py` (читает теперь `0x8000`) печатает последовательность причин: `4,4,4` —
  паника, `5,5,5` — interrupt-wdt, `1,1,1` — power-on/brownout;
* **разбор паники прошлой загрузки** (`diag_report_panic()`): строки «Guru Meditation» идут через
  ROM-консоль и в ящик не попадают, но IDF достаёт из раздела `coredump` сводку
  (`esp_core_dump_get_summary`): задача, PC, `cause`, `vaddr`, обратный стек — это пишется в лог
  при старте, после чего дамп стирается. **ELF и GDB не нужны**: адрес из PC и стека ищется по
  `idf/build/inkmetrics_idf.map`.

## Адреса и раскладка

| Что | Значение |
|---|---|
| Прибор | `192.168.7.1` (HTTP: `/`, `/setup`, `/api/state`, `/ingest` — приём метрик хоста) |
| Хост | адрес выдаёт DHCP самого прибора (обычно `192.168.7.2`); можно поставить статический без шлюза — см. `tools/fix_usb_net.ps1` |
| Режим USB-сети | ECM/RNDIS (NCM Windows не приняла: `CM_PROB_FAILED_INSTALL`) |
| Разделы | `idf/partitions.csv`: app0/app1 под OTA (3 МБ и 1 МБ), `msc` **3,69 МБ** (диск хоста, 7552 сектора, MBR + FAT16) — размер должен совпадать с `MSC_SECTORS` в `msc.c` и с образом из `tools/make_setup_disk.py`; `diag` 64 КБ — «чёрный ящик» логов (по адресу 0x7E0000, занято 8 КБ) |
| Загрузчик (вход) | `BOOT` при подключении USB, держать ~2 с (`GET /api/boot` в загрузчик **не уводит** — проверено) |
| Загрузчик (выход) | после заливки чип остаётся в загрузчике: `esptool --before no_reset --after watchdog_reset chip_id`, иначе передёрнуть USB |
...
3. Отдавать с прибора ту же страницу управления (правка кнопок/страниц), что уже работает
   в Arduino-версии.
4. Wi-Fi точка доступа и OTA.

Сделано 17.09.2026 (чтобы не искать заново): экран — 5 страниц (DEVICE, PING, HOST, SETTINGS,
`HOST SYS`), метрики хоста через `POST /ingest`, диск хоста перенесён в IDF и больше не
«дискета» (3,69 МБ, MBR + FAT16), скрипты настройки ПК лежат в `tools/pc_setup/`.
Что осталось — `ISSUES.md`, раздел 13.
