...
void ingest_apply(const char *json)
{
    ...
    st.gpu_pct = st.gpu_mem_pct = INGEST_NA;
    st.gpu_temp_c = st.cpu_temp_c = INGEST_NA;
    st.up_h = INGEST_NA;
    ...
    if (json_num(json, "gpu_percent", &d)) {
        st.gpu_pct = (float)d;
    }
    if (json_num(json, "gpu_mem_percent", &d)) {
        st.gpu_mem_pct = (float)d;
    }
    if (json_num(json, "gpu_temp", &d)) {
        st.gpu_temp_c = (int)(d + 0.5);
    }
    ...
    ESP_LOGI(TAG, "метрики хоста #%u: cpu %.0f%%, ram %.0f%%, диск %.0f%%, gpu %.0f%% %d C",
             (unsigned)cnt, (double)st.cpu_pct, (double)st.mem_pct, (double)st.disk_pct,
             (double)st.gpu_pct, st.gpu_temp_c);
}
