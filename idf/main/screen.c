 ...
 *   * верхняя строка — состояние ХОСТА по интернету: PING - <мс>MS - <n>/4 (интернет есть,
 *     задержка и счёт от агента), OFFLINE - 0/4 (на пинг не ответили), NO DATA (агент молчит);
 *     рамок вокруг строки и вокруг слова SETUP нет — сняты 23.09;
