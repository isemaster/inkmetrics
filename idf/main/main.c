...
static const char INDEX_HTML[] =
    ...
    /* «Интернет» и «Последние отклики» — про интернет ХОСТА, то есть данные агента,
       ровно те же, из которых прибор собирает верхнюю строку сводного экрана.
       Прежние значения брались с пинга самого прибора (p_target/p_hist): в схеме
       «монитор» у прибора интернета нет, пинг молчит, и страница показывала неправду. */
    "const fmt=(v,u)=>v>=0?(v+u):'-';"
    "const im=s.ingest||{have:0};"
    "const fresh=(im.have&&im.fresh)?1:0;"
    "const online=fresh&&im.ping_known&&im.ping_ok;"
    "const got=im.ping_got;"
    "const uz=im.ping_target?(', узел '+im.ping_target):'';"
    "const ping=!im.have?'нет данных от агента'"
    ":(!fresh?('данные устарели: '+im.age+' с назад')"
    ":(!im.ping_known?'агент интернет не проверял'"
    ":(online?((got<0)?('есть, '+im.ping_ms+' мс (счёт агент не сообщает)'+uz)"
    ":((got>0)?('есть, '+im.ping_ms+' мс, ответили '+got+' из 4'+uz)"
    ":('есть, но пинг молчит: узел отвечает на порт 443'+uz)))"
    ":((got<0)?('нет ответа'+uz):('нет: ни один из четырёх пингов не ответил'+uz)))));"
    "const screen=!fresh?'NO DATA'"
    ":(online?((got<0)?('PING - '+im.ping_ms+'MS')"
    ":((got>0)?('PING - '+im.ping_ms+'MS - '+got+'/4')"
    ":('PING - '+im.ping_ms+'MS - TCP')))"
    ":((got<0)?'OFFLINE':('OFFLINE - '+got+'/4')));"
    "const hist=screen+(im.have?(' · проверено '+im.age+' с назад'+(fresh?'':' (устарело)')):'');"
    ...
    "const met2=(im.tcp>=0?('TCP '+im.tcp):'')"
    "+(im.up_h>=0?((im.tcp>=0?' · ':'')+'аптайм '+im.up_h.toFixed(1)+' ч'):'')"
    "+(im.smart?(' · SMART '+im.smart):'')+(im.count?(' · приёмов '+im.count):'');"
    "document.getElementById('t').innerHTML="
    "`<tr><td>Адрес прибора</td><td>${s.ip}</td></tr>`+"
    "`<tr><td>Интернет (с компьютера)</td><td>${ping}</td></tr>`+"
    "`<tr><td>Как на экране</td><td>${hist}</td></tr>`+"
    ...
    "`<tr><td>Хост: соединения, аптайм, SMART</td><td>${met2}</td></tr>`+"
...
static esp_err_t setup_get(httpd_req_t *req)
{
    ...
    int n = snprintf(page, sizeof(page),
        ...
        "<label>Узел для проверки интернета (слово PING в верхней строке сводного экрана)</label>"
        ...
        " решает, есть ли у хоста интернет: верхняя строка покажет PING - &lt;мс&gt; - &lt;сколько пингов"
        " ответило&gt;, а если ни один не вернулся — OFFLINE. Можно имя или адрес; если пинг не"
        " отвечает, но узел открыт на порту 443, будет TCP.</p>"
