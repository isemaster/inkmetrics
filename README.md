...
Прошивальщик лежит в репозитории: **`flash-web/index.html`** — открыть в Chrome или Edge (с диска
двойным щелчком или через любой локальный веб-сервер). Интернет нужен только чтобы открыть
страницу — дальше она работает с прибором сама.
